...
さらに、集計モードには **Compact Mode（コンパクトモード）** が追加され、合計ポイントと更新ボタンのみを表示する、よりスリムな表示が可能になりました。`Count Target`（集計対象）プロパティでは、`Selection`（選択範囲）、`Parent Section`（親セクション）、`Locked Section`（ロックされたセクション）から選択できます。
- `Selection` を選択した場合は、現在選択されているポイントウィジェットを集計します。便利なショートカットとして、もしポイントウィジェットが明示的に選択されておらず、かつ集計ウィジェットがFigJamのセクション内に配置されている場合、「Count」ボタンを押すと、**そのセクション内のすべてのポイントウィジェットを自動的に集計します**。これにより、手動で選択しなくても素早く合計値を得ることができます。
- `Parent Section` を選択すると、「Count」ボタンを押した際に、**ウィジェットが配置されているセクション内のすべてのポイントウィジェットを自動的に集計します**。
- 新しい `Locked Section` オプションでは、カウンターウィジェットをボード上の特定のセクションにロックできます。これにより、そのセクションが現在選択されているかロックされているかに関わらず、指定されたセクション内のすべてのポイントウィジェットを継続的にカウントできます。これは、FigJamボードの特定の領域での進捗状況を追跡するのに特に役立ちます。
また、ウィジェットは現在の集計範囲（特定のセクション内か、手動選択かなど）を明確にフィードバックします。
