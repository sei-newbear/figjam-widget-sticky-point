...
Additionally, Counting Mode now offers a **Compact Mode** for a more streamlined view, displaying only the total points and a refresh button. The `Count Target` property allows you to choose between `Selection`, `Parent Section`, and `Locked Section`.
- When `Selection` is chosen, it counts the currently selected Point widgets. As a convenient shortcut, if no Point widgets are explicitly selected and the Counter Widget is within a FigJam Section, pressing the 'Count' button will **automatically count all Point widgets within that section**. This allows for quick totals without requiring manual selection.
- When `Parent Section` is selected, pressing the 'Count' button will **automatically count all Point widgets within the section it is placed in**.
- The new `Locked Section` option allows you to lock the Counter Widget to a specific section on your board. This enables the widget to continuously count all Point Widgets within that designated section, regardless of whether the section is currently selected or locked. This is particularly useful for tracking progress in specific areas of your FigJam board.
